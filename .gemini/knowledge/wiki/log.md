...
## [2026-09-20] feat | Hugify 팝업 밝은 테마 및 크기 선택 리디자인

- 팝업을 흰색/하늘색 포인트 테마로 변경하고, 제공 아이콘과 ON/OFF 상태만 남긴 간결한 헤더로 정리했다.
- `90 × 90` 기본값과 `120 × 120` 선택 카드를 추가해 `enabled`/`size` 설정을 저장하고 content script 툴팁 크기에 반영한다.
- 하단에 수땡 방송 바로가기 링크와 아이콘을 추가했다.
